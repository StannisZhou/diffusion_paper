...
idealized (``golf-course'') potential, the estimates are nearly as accurate as the results of direct simulations, but potentially far faster to compute.
\end{abstract}
...
In sum, there are three contributions. The first 
...
And the third is a collection of tools, both analytic and numeric (Monte Carlo), for computing the relevant capacities.
...
\end{split}
\end{align}
where $a(x)=\sigma(x)\sigma(x)^T$ is uniformly elliptic.\footnote{\bf You guys seemed to indicate that these conditions are in fact both sufficient and necessary. Do you want to add, say here, a footnote or comment to that effect, and maybe bring back the reference to Pavliotis?}
...
The key to going from an assumed flat landscape outside of the immediate neighborhoods of targets to the conclusion that first-passage probabilities depend only on certain local capacities is the intermediate conclusion that in the small-target (or large-dimension) limit, those probabilities are nearly independent of starting location, i.e. $\varepsilon$-flat relative to all of $\Omega$, excepting some small neighborhoods around the targets.
Sufficient conditions for this $\varepsilon$-flatness is the purpose of our first result, and the {\em assumption} of 
$\varepsilon$-flatness is the main hypothesis of our second result, which then provides an explicit formula for the first-passage probabilities in terms terms of certain local capacities.
...
$A \subset\dA\subset\tA\subset\Omega$, and 
$B \subset\dB\subset\tB\subset\Omega$.
...
The experimental setup is not so big as to preclude
...
``walk-on-spheres'' method to simulate trajectories in the flat region,\cite{bingham1972random} JIT compilation to remove loop overhead, multi-CPU parallelization, and the coarsest time step that yielded accurate results. As for capacity estimation, we made no effort to adjust to number of samples or the discretization parameters. Under these conditions, a single run of the diffusion took about as long as estimating the two capacities. On the other hand, the capacity estimation algorithm is trivially parallelized with minimal overhead, likely resulting in
several orders-of-magnitude improvement.\footnote{\bf OK? Reword?}

%The purpose of our approach is to circumvent direct simulation, the feasibility of which is severely challenged by large entropic barriers. To what extent, then, is the approximation more efficient? In the current experiments, we compared the time it takes to estimate the hitting probabilities using direct simulations with the time required by our approach using capacity estimation. For the comparison we made a best effort to implement a fast simulator, by using  the ``walk-on-spheres'' method to simulate trajectories in the flat region,\cite{bingham1972random} JIT compilation to remove loop overhead, multi-CPU parallelization, and the coarsest time step that yielded accurate results. We obtained the following CPU-time comparisons:
%
%\begin{description}
...
\newpage
\appendix
\noindent {\bf APPENDIX}\footnote{\bf Seem to have broken your appendix environment. In fact, everything has gotten a little brittle---witness the huge spaces that appear without warning. The appendix needs some work. In general, I changed wording and notation in many places in the main text and didn't make the corresponding changes everywhere in the appendix. But I did make a lot of them, so definitely best to work from this copy. Things to fix: CHop isn't used in the main text and probably shouldn't be used here; I didn't systematically change all of the instances of $d_A$ and $d_B$; I DID replace the theorems and propositions so that they match the new statements in the main text, but I didn't delve into all of the proofs to make the corresponding changes.}
...
It turns out that by taking $n$ to be sufficiently high or $d_A,d_B$ to be sufficiently small, we can make the hitting probabilities arbitrarily close to constant in the region away from $\tilde A,\tilde B$.  This is the content of Theorem \ref{thm:epsilon_flat} from the main text, which we restate here for the convenience of the reader:
...
\]
%
for arbitrarily small $\varepsilon$ and arbitrarily large $t$, by taking $n$ sufficiently high or $d_A,d_B$ sufficiently small.
\end{enumerate}
Putting these two results together at $t=2/\varepsilon$ we obtain that we can make $|h_{A,B}(x) - \mathbb{E}[h_{A,B}(Z)]| \leq \varepsilon / 2 + \varepsilon/2 = \varepsilon$ for arbitrarily small $\varepsilon$ by taking $n$ sufficiently high or $d_A,d_B$ sufficiently small.
\end{proof}
...
Thus, to complete our proof, it suffices to show that we can drive $L(d_A, \tilde d_A, r_{\tilde A},n)$ to zero by taking $d_A$ small or $n$ large:
\begin{itemize}
    ...
    \item When $n$ is large.  It is clear that the previous result holds for any value of $\tilde d_A\in (d_A, r_{\tilde A})$.  For the large-$n$ case, we will be more picky: we will take $\tilde d_A=\sqrt{d_A r_{\tilde A}}$.  
...
Let us now review our arguments to see how the vanishing of $L$ leads to our desired conclusion.  We have just shown that by taking $n$ large or $d_A$ small, we can ensure that $L$ vanishes.  Thus for any $x_0\notin \tilde A$ and any $\varepsilon'>0$, we can ensure
\begin{align*}
...
{\algorithm{\label{alg:hitting_prob_estimation}Estimating $h_{R,\tilde R^c}(x)$ for many values of $x$ along a shell $\partial S$
