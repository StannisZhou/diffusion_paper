...
Reversible diffusions play a key role in a wide variety of physical systems.  For example, the folding of macromolecules into their native configurations is often posed as a diffusion (either directly through simulations of atomic dynamics or indirectly through mesoscopic models \cite{Scheraga2007-qw,Hospital2015-ol,lei2010direct}), the fluctuation of chemical species in solution can be modeled as diffusions (``chemical Langevin equations'' are one classic example of this approach \cite{sotiropoulos2011analytical,gillespie2000chemical}), the motion of particles through  membranes can be posed as a diffusion \cite{holcman2004escape}, and so-on.   

In this paper we seek to estimate first-passage hitting probabilities of such diffusions: given an initial condition $X_0$ and two targets, $A,B$, what is the probability that we will hit target $A$ first?   The best approach to this problem will be one which is tailored to the specifics of the diffusion's dynamics.

Here we focus on dynamics where the diffusion can ``forget'' its initial condition before either target is hit.  More formally, we will consider a large region $M$.  The targets will lie outside of $M$ and the diffusion will be initialized inside $M$; we are interested in the case that the rate of ergodicity inside of $M$ is faster than the typical escape time from $M$.   

If the system forgets its initial conditions in this way, we will say the diffusion reaches ``metastable equilibrium'' in $M$.  Figure \ref{fig:ToyModel} gives a rough sketch; we will make these ideas rigorous in \S\ref{sec:Preliminaries}.  

When the first passage time is very long, many standard methods for approaching the problem break down; the associated partial differential equations often include large Lipschitz constants, and direct Monte-Carlo simulation requires prohibitive number of timesteps (cf.\ \cite{Baum1986-we, Wille1987-tf, Machta2009-gh}).  Some authors take a pessimistic view on this subject: ``If these processes are intrinsically slow, i.e. require an extensive sampling of state space, not much can be done to speed up their simulation without destroying the dynamics of the system'' \cite{Christen2008-ge}.  In fact, the separation of timescales can sometimes make analysis easier.   The literature includes a variety of tools which use this idea to advantage.  Here we review a few of these tools:
...
\begin{figure}
    ...
    \caption{\footnotesize\linespread{1.}\selectfont{} {\bf First passage capacities allow efficient computation of hitting probabilities.} A reversible stochastic differential equation in an $n$-dimensional state-space is initialized at position $x_0$.  What is the probability it will reach region $A$ first before $B$?  We consider the case that $X$ reaches a metastable equilibrium before hitting either target.  Theorem \ref{thm:main_thm} then shows that the hitting probability can be approximated to within $\sqrt{2\epsilon}$ using only ``first passage capacities.'' These capacities can be computed using local simulations in neighborhoods around the targets.  Above we show several examples of this kind of problem.  In each case, the first passage capacities can be computed using only local simulations performed in the gray areas.}
\label{fig:ToyModel}
\end{figure}
...
It turns out that these hitting probabilities are much easier to estimate if the process reaches a metastable equilibrium before hitting either target.  In particular, we will assume that the process forgets its initial condition to such an extent that the hitting probabilities are nearly constant for any initial condition inside a set $M$:
...
A proof can be found in Appendix \ref{sec:proof_epsilon_flat}.  The essence of the proof is that the first exit from $\Omega \backslash \tilde A\cup \tilde B$ is long relative to the mixing time of the process.  The $\varpepsilon$-flatness condition will always hold in this situation.
