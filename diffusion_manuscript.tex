...
When $\varepsilon$-flatness holds, these capacities are intimately connected to hitting probabilities:

\begin{theorem}\label{thm:main_thm}  Let $X$ be a reversible stationary diffusion on $\Omega$ governed by Equation (\ref{equ:general_sde}).  Let $A\subset\tilde A\subset\Omega,B\subset\tilde B\subset\Omega$.  If $\tilde A,\tilde B$ are disjoint and $\Omega \backslash (\tilde A \cup \tilde B)$ is $\varepsilon$-flat for the hitting probability function $h_{A,B}(x)$, then 
	\[ \sup_{x \not\in \tilde{A}, \tilde{B}} \left| h_{A,B} (x) - \frac{\ensuremath{\operatorname{cap}} (A,
...
For a stationary reversible  diffusion process $X$ in $\Omega$ with
