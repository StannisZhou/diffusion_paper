...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 \section{Numerical Experiments\footnote{All the experimental results can be reproduced or easily modified from open-source code, which can found, along with detailed instructions, at \url{https://github.com/StannisZhou/capacity_hopping}.}}
...
To get some crude sense of how these two approaches compare in terms of speed, we make some illuminating calculations based on the results in \S\ref{sec:nontrivial_results}. Use $p^{(i)}_A, i=1, \cdots, 100$ to denote the estimated hitting probabilities for the $100$ different initial configurations using direct simulations, and use $p_A$ to denote the estimated hitting probabilities using the capacity-estimation approach. We use
\begin{equation*}
	\frac{1}{100}\sum_{i=1}^{100}(p^{(i)}_A - p_A)^2\approx 0.0004
\end{equation*}
to quantify the uncertainty in $p_A$. Now assume we want to use direct simulations to estimate the hitting probability starting from a particular initial configuration. In the extreme case where the true hitting probability is $0.9$ (which is much larger than $0.8460$, the maximum we get in our actual experiments), using the formula for the variance of the mean of i.i.d. Bernoulli random variables, we would need approximately $\frac{0.9 \times (1 - 0.9)}{0.0004} \approx 225$ direct simulations to reach the same level of accuracy as the capacity-estimation approach. This indicates that the capacity-estimation approach is orders of magnitude faster than direct simulations.
