...
\begin{description}[noitemsep]
	...
	\item[Application] Inspired by the theoretical results, we present CHop as a general method to estimate hitting probabilities of small targets in the presence of entropic barriers. We verify the effectiveness of CHop through numerical experiments.
\end{description}
...
In this section, we introduce the concept of $\varepsilon$-flatness to formalize the idea that a function is ``approximately constant.'' We establish $\varepsilon$-flatness for hitting probabilities in our toy model, and see when it holds for more general models.
%
...
Let $X$ be a reversible stationary process governed by Equation (\ref{equ:general_sde}).  Then for $A \subset \tilde{A} \subset \Omega$, we will define the capacity $\ensuremath{\operatorname{cap}} (A, \tilde{A})$ for $X$ as
%
\[ \ensuremath{\operatorname{cap}} (A, \tilde{A}) \triangleq \int_{\tilde A \backslash A}
||\sigma(x) \nabla h_{A, \tilde{A}^c}(x)||^2 e^{- U(x)} \mathrm{d} x \]
%
where $h_{A, \tilde{A}^c}(x)$ denotes the probability that $X$ will hit $A$ before hitting $\tilde{A}^c=\Omega \backslash \tilde A$, given $X_0=x \in \tilde A\backslash A$.  
\end{definition}

We refer the reader to Appendix \ref{sec:reversible_diffusion} for more details on capacity and the related concept of Dirichlet form.  Note that there are several related definitions of `capacity' in the literature.  Throughout this work we will always use `capacity' to indicate the above definition.

In the presence of $\varepsilon$-flatness, these local capacities are intimately related to global hitting probabilities.  Let $A\subset\tilde A\subset\Omega,B\subset\tilde B\subset\Omega$.  If $\tilde A,\tilde B$ are disjoint and $\Omega \backslash (\tilde A \cup \tilde B)$ is $\varepsilon$-flat for the hitting probabilities $h_{A,B}(x)$, then we obtain
...
In terms of these definitions, we obtain a natural generalization for Theorem \ref{thm:main_thm}:
...
\begin{enumerate}[noitemsep]
    \item Golf-course potentials enjoy $\varepsilon$-flat hitting probability functions.  We let $h(x)$ denote the probability for process $X$ to hit a particular target first, given that the process began at point $x$.  We saw that there was a $\varepsilon \ll 1$, a large set $M$, and a constant $\bar h$ such that $|h(x)-\bar h|<\varepsilon$ for every $x\in M$.  That is, the hitting probability is nearly equal to $\bar h$ for every initial condition in $M$.
    \item We showed that `CHop probabilities' give estimates for the hitting probability $\bar h$.  The estimation error is always less than $\varepsilon +\sqrt{\varepsilon/2}$.  These CHop probabilities can be calculated using only local simulations around each target.
\end{enumerate}
...
Use $X$ to denote a process governed by Equation (\ref{equ:general_sde}) on $\Omega$.  Let $A_1\cdots A_n \subset \Omega$ denote the targets of interest.  The CHop method estimates the distribution for which target $X$ hits first.  More formally, we let $h_{A_k,\cup_{i\neq k} A_i}(x)$ denote the probability that the process $X$ with initial condition $X_0=x$ hits target $A_k$ before any other target.  The CHop method gives estimates for $h_{A_k,\cup_{i\neq k} A_i}(x)$ for initial conditions $x$ which are modestly far away from any target.  It consists of the following steps:

\begin{enumerate}[noitemsep]
    \item Identify suitable neighborhood sets $\tilde A_k \subset \Omega$ where $A_k \subset \tilde{A}_k, k=1, \cdots, n$.  The accuracy of the method will be determined by the flatness of the hitting probability functions $h_{A_k,\cup_{i\neq k} A_i}(x), k=1, \cdots, n$ on $M = \Omega \backslash \bigcup_{k = 1}^n \tilde{A}_k $.  Flat functions will guarantee that the estimates is accurate.
    \item For each target $k$, use simulations to estimate the local capacity ${\operatorname{cap}} (A_k, \tilde{A}_k)$.  Let $\hat \rho_k$ denote the value of this estimate.  
    \item For any initial condition $x \in M$ and any target $k$ we estimate
\begin{equation*}
h_{A_k,\cup_{i\neq k} A_i}(x) \approx \frac{\hat \rho_k}{\sum_{i = 1}^n \hat \rho_i}
\end{equation*} 

\end{enumerate}

The second step provides the only computational challenge, because analytical computation of capacities is not generally possible.  The remainder of Section \ref{sec:algorithm} is devoted to describing a new method for capacity estimation.
...
\tilde{G}$
\end{enumerate}
The capacity can then be estimated as
...
In this section we use simulations to test the limits of our theories and algorithms.  We would like to test each of the following ideas:
...
\item The capacity estimation algorithm is accurate.
\item The overall CHop method is accurate and significantly faster than naive simulation.
\end{itemize}

In what follows, we present experimental results for all of these points.  We consider dynamics with both a flat and a nontrivial energy landscape.  All the results presented in this section can be reproduced from open-source code; for detailed instructions, please refer to \url{https://github.com/StannisZhou/capacity_hopping}.
...
\end{equation*}
for target $A$.  That is, the target $A$ is a ball centered at $x_A$ with radius $r_A$, and we use a neighborhood of size $r_\tilde A$ to perform the local capacity calculations.  Similarly for target $B$, we have:
\begin{equation*}
x_B = \begin{pmatrix}%
-0.7&0.0&0.0&0.0&0.0%
\end{pmatrix},
...
In the toy model, we see that the $\varepsilon$-flatness condition indeed holds, with $\varepsilon \approx 0.0365$.  We ran 2000 diffusion simulations at each of the 100 randomly selected initial conditions in the region $\mathcal{B}(0, 1) \setminus (\tilde{A} \cup \tilde{B})$. This region represents locations that are modestly far away from the targets.  A histogram of these probabilities may be found in Figure \ref{fig:results}.  From this we see that the hitting probabilities are approximately a constant, regardless of the different initial conditions in this region. They all fall in a fairly narrow range, namely $[0.0820, 0.1185]$.  Thus the hitting probabilities are indeed approximately constant.


\subsubsection{The hitting probabilities are well-approximated using CHop probabilities}
...
According to this formula, the CHop probability is $0.1100$.  This probability is within $2\%$ of the value $0.0975$ found by the simulations in the previous section.  Thus the CHop probability gives a reasonable approximations to the hitting probability.
...
These show reasonable agreement between the truth and our estimates.
...
r_{\tilde{B}} = 0.15
\end{equation*}
for target $B$. Note that that these simulations feature extra parameters, $d_A,d_B$.  These indicate the size of the regions where the energy landscape is nontrivial.  We refer the reader to Appendix \ref{sec:energy_function} for more details on the energy used.
...
Using our capacity estimation algorithm from Section \ref{sec:outlinecapacest}, we obtained an estimate of $0.8360$ for the CHop probability of this model.  This is within 2\% of the average hitting probability $0.8175$ found in our direct simulations.  For the capacity estimation algorithm, we used the parameters $ m = 2, N_p = 3000, N_b = 5, N_s = 1000$ and a time-step of $1 \times 10^{-6}$.  For target $A$ we used $n = 4$ and for the slightly larger target $B$ we used $n = 5$.  

\subsubsection{Capacity estimation algorithm}

For a process with nontrivial energy landscape it is difficult to assess the accuracy of the capacity estimation algorithm.  It is not possible to obtain exact values for the capacity.  Therefore, we cannot directly test whether our algorithm is accurately estimating the capacities.  However, the previous section does offer some indirect evidence: the capacity estimation algorithm yields CHop probability estimates which accurately reflect the true hitting probabilities. 

We would also like to test the speed of the capacity estimation algorithm.  We compared the time it takes to estimate the hitting probabilities using direct simulations with the time our capacity estimation algorithm needs to estimate the capacities for all targets. Note that in order to make the direct simulations feasible for the $5$-dimensional toy model we are working on, we made our best effort to make the direct simulation fast.  We used an efficient walk-on-spheres method to simulate trajectories in the flat region,\cite{bingham1972random} JIT compilation to remove loop overhead, multi-CPU parallelization, and the coarsest time step that yielded accurate results.  The CPU times for direct simulations and the CHop method are shown below:
...
\item[CHop] 50.31 seconds (on a single CPU). In doing this we employed a more accurate time step than the one used for direct simulations ($10^{-6}$ in particular). Despite this, the CHop method was 82 times faster on a single CPU than the direct simulations on 8 CPUs.

\end{description}

This reflects a 662-fold acceleration. The times reported above reflect a particular choice of parameters, but we found that the accuracy of the algorithm was fairly robust to choices of $m, n, N_p, N_b$ and $N_s$, as long as they are reasonably large. Furthermore note that the time-consuming elements of the capacity estimation algorithm are ``embarrassingly parallelizable'' and it should be easy to further accelerate our capacity estimation algorithm using parallelization.  There was no need to do so in this case because we could easily run the CHop method on a laptop.  However, larger scale problems would certainly benefit from this kind of additional acceleration.

\section{Conclusions and Discussions}\label{sec:discussion}

In this work, we give a new analysis of ``entropic barriers'' by taking a hitting-probability perspective. While lacking timing information, hitting probabilities offer more kinetic understanding than purely thermodynamic analyses. We show conditions where hitting probabilities exhibit an approximate invariance to initial conditions, as long as the initial conditions are modestly apart from target regions. It turns out that this invariance further implies an invariance to the energy landscape away from the targets. This means that the hitting probabilities can be calculated using only local simulations around the targets. These insights are used to develop CHop, an algorithm to estimate hitting probabilities of small targets in the presence of entropic barriers. Numerical experiments on a prototypical entropic barrier, a toy model with a golf-course potential, demonstrate the effectiveness of CHop.

Entropic barriers, which arise because of the difficulty of reaching a small number of target configurations out of a vast number of possible configurations, are in contrast to \emph{enthalpic barriers}, which arise because of the difficulty of escaping local minima. The challenges of molecular dynamics result from the complicated energy landscapes associated with different biomolecular systems, and can largely be summarized by these two different kinds of barriers. However, careful inspection of the existing methods shows that a majority of the previous developments focus on enthalpic barriers, where the central picture consists of an energy landscape with a multitude of local minima separated by high energetic barriers. A less studied picture involves the entropic barriers, where the energy landscapes includes large flat regions.  With a better understanding of both kinds of barriers, hopefully we will be able to achieve faster, more accurate simulations of large dynamical systems.
